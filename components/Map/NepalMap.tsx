...
export default function NepalMap({ }: NepalMapProps) {
    ...
    const [byelectionResults, setByelectionResults] = useState<ElectionResultsMap | null>(null);
    ...
    const [showByelection, setShowByelection] = useState(false);
    const [selectedId, setSelectedId] = useState<string | null>(null);
    const [isAnimating, setIsAnimating] = useState(false);
    const [flickeringLayers, setFlickeringLayers] = useState<Set<string>>(new Set());
    const [previousByelectionState, setPreviousByelectionState] = useState(false);
    ...
    useEffect(() => {
        ...
        fetch('/data/byelection_results.json')
            .then(r => r.json())
            .then((data: Record<string, ElectionResult>) => {
                // Normalize keys to lowercase for matching with GeoJSON IDs
                const normalized: ElectionResultsMap = {};
                ...
                setByelectionResults(normalized);
            })
            .catch(err => console.error("Error loading byelection results:", err));
    }, []);
    ...
    const getFeatureStyle = useCallback((feature: import('geojson').Feature<import('geojson').Geometry, ConstituencyProperties> | undefined) => {
        ...
        if (electionYear === '2079' && feature?.properties?.id) {
            let result = electionResults2079?.[feature.properties.id];
            
            // If byelection is active and this constituency has byelection results, use those
            if (showByelection && byelectionResults?.[feature.properties.id]) {
                result = byelectionResults[feature.properties.id];
            }
    ...
    }, [isSatellite, selectedId, electionYear, electionResults2079, showByelection, byelectionResults]);

    // Flicker animation for byelection districts
    const flickerByelectionDistricts = useCallback(() => {
        if (!geoJsonRef.current || !byelectionResults || electionYear !== '2079') return;

        const layers = geoJsonRef.current.getLayers();
        const byelectionLayerIds = new Set(Object.keys(byelectionResults));

        layers.forEach((layer) => {
            const l = layer as L.Layer & { 
                feature?: import('geojson').Feature<import('geojson').Geometry, ConstituencyProperties> 
            };
            
            if (l.feature?.properties?.id && byelectionLayerIds.has(l.feature.properties.id)) {
                const feature = l.feature;
                setFlickeringLayers(prev => new Set(prev).add(feature.properties!.id));
                
                let flickerCount = 0;
                const maxFlickers = 5;
                const flickerDuration = 300; // 300ms per flicker (total ~3 seconds)
                
                const flicker = () => {
                    if (flickerCount >= maxFlickers) {
                        // Final state based on activation
                        if (l instanceof L.Path) {
                            const finalStyle = getFeatureStyle(feature);
                            l.setStyle(finalStyle);
                        }
                        setFlickeringLayers(prev => {
                            const newSet = new Set(prev);
                            newSet.delete(feature.properties!.id);
                            return newSet;
                        });
                        return;
                    }
                    ...
                    if (l instanceof L.Path) {
                        // Calculate original and target colors
                        const original2079Result = electionResults2079?.[feature.properties!.id];
                        const byelectionResult = byelectionResults?.[feature.properties!.id];
                        
                        const originalWinner = original2079Result?.candidates?.find((c: { status: string; }) => c.status === 'Elected');
                        const byelectionWinner = byelectionResult?.candidates?.find((c: { status: string; }) => c.status === 'Elected');
                        
                        const originalColor = originalWinner?.party ? getPartyColor(originalWinner.party) : '#FFFFFF';
                        const targetColor = byelectionWinner?.party ? getPartyColor(byelectionWinner.party) : originalColor;
                        
                        // Determine what we're transitioning from and to using previous state
                        const isActivating = showByelection && !previousByelectionState;
                        const toColor = isActivating ? targetColor : originalColor;
                        
                        let style;
                        if (flickerCount === 0) {
                            // First frame: keep original color (no change needed)
                            return;
                        } else if (flickerCount % 2 === 0) {
                            // Even frames (after first): white
                            style = {
                                fillColor: '#FFFFFF',
                                color: '#D1D5DB',
                                weight: 2,
                    ...
                    flickerCount++;
                    setTimeout(flicker, flickerDuration);
                };
                
                flicker();
            }
        });
    }, [byelectionResults, electionYear, electionResults2079, showByelection, previousByelectionState, getFeatureStyle]);

    // Trigger flicker animation when byelection toggle changes
    useEffect(() => {
        if (byelectionResults && electionYear === '2079') {
            flickerByelectionDistricts();
            // Update previous state after animation starts
            setPreviousByelectionState(showByelection);
        }
    }, [showByelection, byelectionResults, electionYear, flickerByelectionDistricts]);
    ...
    const createTooltipContent2079 = (properties: ConstituencyProperties) => {
        let result = electionResults2079?.[properties.id];
        const byelectionResult = byelectionResults?.[properties.id];
        const name = properties.name;
        const isByelectionActive = showByelection && byelectionResult;
        
        // Use byelection result if active
        if (isByelectionActive) {
            ...
            <div class="min-w-[240px] font-sans">
                <div class="font-bold text-sm text-gray-900 border-b pb-1 mb-2 sticky top-0 bg-white z-10 flex justify-between items-center">
                    ...
                    <div class="flex gap-1 items-center">
                        ...
                        ${isByelectionActive ? '<span class="text-[9px] font-normal text-orange-600 border border-orange-200 px-1 rounded">BYELECTION</span>' : ''}
                    </div>
                </div>
                ${isByelectionActive && byelectionResult?.reason ? `
                    <div class="text-[9px] text-orange-600 mb-2 p-2 bg-orange-50 rounded border border-orange-200">
                        <div class="font-medium mb-1">By-election Reason:</div>
                        <div>${byelectionResult.reason}</div>
                        <div class="text-[8px] text-orange-500 mt-1">Date: ${byelectionResult.election_date}</div>
                    </div>
                ` : ''}
    ...
    // Update tooltips when byelection state changes
    useEffect(() => {
        if (geoJsonRef.current && electionYear === '2079') {
            const layers = geoJsonRef.current.getLayers();
            layers.forEach((layer) => {
                const l = layer as L.Layer & { 
                    feature?: import('geojson').Feature<import('geojson').Geometry, ConstituencyProperties> 
                ...
                if (l.feature?.properties) {
                    const tooltipContent = createTooltipContent2079(l.feature.properties);
    ...
    }, [showByelection, byelectionResults, electionYear, createTooltipContent2079]);
    ...
    const onEachFeature = (feature: import('geojson').Feature<import('geojson').Geometry, ConstituencyProperties>, layer: L.Layer) => {
        ...
        layer.on({
            mouseover: (e) => {
                ...
                // Don't apply hover style if this layer is currently flickering
                if (feature.properties.id && flickeringLayers.has(feature.properties.id)) return;
            ...
            mouseout: (e) => {
                const l = e.target;
                // Don't reset style if this layer is currently flickering
                if (feature.properties.id && flickeringLayers.has(feature.properties.id)) return;
    ...
    return (
        <div className="h-full w-full bg-white relative">
            ...
            <MapControls
                ...
                showByelection={showByelection}
                onToggleByelection={() => setShowByelection(!showByelection)}
            />
